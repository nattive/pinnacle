...
import { playCourse, userHasTakenQuiz } from "../../Actions/courseAction";
...
function Play(props) {
  ...
  useEffect(() => {
    if (
      ...
      props.course.Modules[0] &&
      props.course.Modules[0].course_materials[0]
    ) {
      dispatch({
        type: PLAY_MODULES,
        payload: props.course.Modules[0].course_materials[0],
      });
  ...
  useEffect(() => {
   ...
   props.userHasTakenQuiz(props.modulePlaying.id);
  }, [props.modulePlaying]);
  // setModule
  return (
    <>
      <div className="row bg-light no-gutters" style={{ top: "5em" }}>
        <div className="col-xs-12 col-sm-12 col-md-8 w-100">
          {playing.videoPath ? (
            <VideoPlayer videoUrl={playing.videoPath} banner={playing.images} />
          ) : (
            <Skeleton height={500} />
          )}
          {playing.id ? (
            <CourseDetails {...playing} />
          ) : (
            <Skeleton height={300} />
          )}
        </div>
...
export default connect(mapStateToProps, { playCourse, userHasTakenQuiz })(Play);
